...
Main fills `init-run`'s optional arguments from the task description. When the task requires that an existing specification, function contract, or case-lib content stay unchanged -- for example "do not alter `solver`'s `Require`/`Ensure`" or "you may only append to the lib" -- pass those function names as `--freeze-spec <function>`. That flag is the only mechanical enforcement of such a constraint; stating it in the task text alone binds nothing. Omitting it lets the annotation agent rewrite specifications freely.

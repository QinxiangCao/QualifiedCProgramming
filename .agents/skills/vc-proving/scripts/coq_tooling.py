...
FIXED_LOAD_PATH_MAPPINGS: tuple[tuple[str, str, str], ...] = (
    ("-R", "SeparationLogic/flocq/src", "Flocq"),
...
PROOF_DECLARATION_RE = re.compile(
    r"^(Lemma|Theorem|Proposition|Corollary|Example|Fact|Remark)\s+([A-Za-z0-9_']+)\s*:",
    re.MULTILINE,
)
...
def target_case_sources(
    ...
    *,
    ...
    sources = [directory / f"{case_name}{suffix}" for suffix in TARGET_CASE_MODULE_SUFFIXES]
    # Some generated cases include local strategy modules in the same formal
    # directory.  Those modules can depend on the case lib, so their old .vo
    # files must not be trusted after the lib is overlaid/recompiled.
    for root in [target_rel, *(overlays or {})]:
        root_dir = root.parent
        if root_dir != directory:
            continue
        for sibling in sorted(root_dir.glob("*.v")):
            rel = Path(sibling.as_posix())
...
def _path_spellings(path: Path) -> set[str]:
    ...
    if text.endswith(".v"):
        stem = text[:-2]
        variants.update({stem, stem.replace("/", "\\"), stem.replace("\\", "/")})
    return variants
...
def normalize_generated_case_imports(
    *,
    ...
    """Repair generated absolute-path module imports in build-workspace copies."""

    workspace_root = workspace_root.expanduser().resolve()
    build_workspace = build_workspace.expanduser().resolve()
    sources = [Path(path.as_posix()) for path in current_case_sources]
    replacements: dict[str, str] = {}
    for rel in sources:
        module = rel.stem
        for spelling in _path_spellings(workspace_root / rel):
            replacements[spelling] = module
        for spelling in _path_spellings(build_workspace / rel):
            replacements[spelling] = module
        replacements[rel.as_posix()[:-2] if rel.as_posix().endswith(".v") else rel.as_posix()] = module
    changed: list[str] = []
    for rel in sources:
...
def normalize_local_case_module_references(
    *,
    ...
    """Qualify same-directory target-case Require/Include references."""

    build_workspace = build_workspace.expanduser().resolve()
    sources = [Path(path.as_posix()) for path in current_case_sources]
    by_directory: dict[Path, dict[str, str]] = {}
    for rel in sources:
        logical = relative_to_logical_module(rel)
        if logical is None:
            continue
        by_directory.setdefault(rel.parent, {})[rel.stem] = logical
    ...
    for rel in sources:
        ...
        if not path.is_file():
            continue
        local = by_directory.get(rel.parent, {})
        if not local:
            continue
        text = path.read_text(encoding="utf-8")
        ...
        for line in text.splitlines(keepends=True):
            ...
            require = re.match(r"^(\s*)Require\s+(Import|Export)\s+([A-Za-z0-9_'\s]+)\.(\s*)$", body)
            if require is not None:
                indent, mode, modules_text, trailing = require.groups()
                modules = modules_text.split()
                logicals = [local.get(module) for module in modules]
                if modules and all(logicals):
                    prefixes = {str(logical).rsplit(".", 1)[0] for logical in logicals}
                    if len(prefixes) == 1:
                        updated_lines.append(
                            f"{indent}From {prefixes.pop()} Require {mode} {' '.join(modules)}.{trailing}{newline}"
                        )
                        touched = True
                        continue
            include = re.match(r"^(\s*)Include\s+([A-Za-z0-9_']+)\.(\s*)$", body)
            if include is not None and include.group(2) in local:
                indent, module, trailing = include.groups()
                updated_lines.append(f"{indent}Include {local[module]}.{trailing}{newline}")
                touched = True
                continue
...
def remove_auto_manual_proof_overlaps(
    *,
    ...
    """Drop build-only proof_auto blocks shadowed by proof_manual witnesses.

    Generated proof_auto files can contain placeholder lemmas for witnesses that
    are also assigned to the manual file.  The final goal_check includes both
    modules into one VC_Correctness module, so the build workspace must keep the
    manual declaration and suppress the duplicate auto placeholder.
    """

    build_workspace = build_workspace.expanduser().resolve()
    sources = {Path(path.as_posix()) for path in current_case_sources}
    changed: list[str] = []
    for auto_rel in sorted(path for path in sources if path.name.endswith("_proof_auto.v")):
        manual_rel = auto_rel.with_name(auto_rel.name.replace("_proof_auto.v", "_proof_manual.v"))
        if manual_rel not in sources:
            continue
        auto_path = build_workspace / auto_rel
        manual_path = build_workspace / manual_rel
        if not auto_path.is_file() or not manual_path.is_file():
            continue
        manual_names = {
            match.group(2)
        ...
        def keep_or_drop(match: re.Match[str]) -> str:
            return "" if match.group(1) in manual_names else match.group(0)

        updated = PROOF_BLOCK_RE.sub(keep_or_drop, auto_text)
        if updated != auto_text:
            auto_path.write_text(updated, encoding="utf-8")
    ...
    *,
    ...
    def visit(rel: Path) -> None:
        ...
        for module in _required_modules(path.read_text(encoding="utf-8")):
            ...
            if dep is None:
                same_directory = rel.parent / f"{module}.v"
                if (build_workspace / same_directory).is_file():
                    dep = same_directory
                else:
                    dep = _local_alias_wrapper(build_workspace, rel, module)
            if dep is not None and (build_workspace / dep).is_file():
                if dep in current_sources or dep.parent == Path(".") or not (build_workspace / dep.with_suffix(".vo")).is_file():

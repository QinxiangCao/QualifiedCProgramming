...
Proof.
  ...
  sep_apply
    (UIntArray.undef_seg_merge_to_undef_seg ptr (Zabs signed_size) mid high);
    try lia.
  sep_apply
    (UIntArray.undef_seg_merge_to_undef_seg ptr (Zabs signed_size) high cap);
    try lia.
  sep_apply (store_Z_from_fields tr ptr signed_size cap r);
    [ | entailer! | entailer!].
  entailer!.
Qed.
...
Proof.
  ...
  - right. split; [lia |].
    ...
    assert (Hn_bad : n <= P * D - 1).
    {
      ...
      {
        replace (P * d - 1) with ((P - 1) * d + (d - 1)) by ring.
        lia.
      }
...
Proof.
  ...
  assert (Hpow_pos : 0 < UINT_MOD ^ (dn0 + i)).
  { apply Z.pow_pos_nonneg; unfold UINT_MOD; lia. }
...
Proof.
  ...
  entailer!.
  sep_apply (UIntArray_full_to_mpd_store_Z_compact dp 1 l_dp d); try assumption.
  entailer!.
Qed.
...
Proof.
  ...
  Exists shift d1 d0 di.
  entailer!.
  sep_apply (UIntArray_full_to_mpd_store_Z_compact dp 2 l_dp d); try assumption.
  entailer!.
Qed.
...
Proof.
  ...
  - sep_apply (store_preinv_divisor_gt2_project dp inv dn d Hdn).
    ...
    sep_apply (UIntArray_full_to_mpd_store_Z_compact dp dn l_dp d);
      try assumption.
    entailer!.
    csimpl. entailer!.
    rewrite Hshift0. entailer!.
    subst shift.
    unfold div_inverse_store, div_inverse_fields.
    destruct (Z.eq_dec dn 1); [lia |].
    entailer!.
    replace shift with 0 in Hvalid_store by lia.
    exact Hvalid_store.
Qed.

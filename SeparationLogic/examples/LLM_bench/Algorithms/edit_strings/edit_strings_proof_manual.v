...
Lemma proof_of_max_edit_string_matches_entail_wit_4_split_goal_1 : max_edit_string_matches_entail_wit_4_split_goal_1.
Proof.
  pre_process.
Qed.

Lemma proof_of_max_edit_string_matches_entail_wit_4_split_goal_2 : max_edit_string_matches_entail_wit_4_split_goal_2.
Proof.
  pre_process.
Qed.

Lemma proof_of_max_edit_string_matches_entail_wit_4_split_goal_3 : max_edit_string_matches_entail_wit_4_split_goal_3.
Proof.
  pre_process.
Qed.

Lemma proof_of_max_edit_string_matches_entail_wit_4_split_goal_4 : max_edit_string_matches_entail_wit_4_split_goal_4.
Proof.
  pre_process.
Qed.

Lemma proof_of_max_edit_string_matches_entail_wit_4_split_goal_5 : max_edit_string_matches_entail_wit_4_split_goal_5.
Proof.
  pre_process.
  dump_pre_spatial.
  eapply EditZeroFull_scratch_bound__zeroing_and_base_build; try lia;
    eapply EditZeroPrefix_to_full_at_bound__zeroing_and_base_build; eauto; lia.
Qed.

Lemma proof_of_max_edit_string_matches_entail_wit_4_split_goal_6 : max_edit_string_matches_entail_wit_4_split_goal_6.
Proof.
  pre_process.
  dump_pre_spatial.
  eapply EditZeroPrefix_to_full_at_bound__zeroing_and_base_build; eauto; lia.
...
Lemma proof_of_max_edit_string_matches_entail_wit_4_split_goal_7 : max_edit_string_matches_entail_wit_4_split_goal_7.
Proof.
  pre_process.
  dump_pre_spatial.
  eapply EditZeroPrefix_to_full_at_bound__zeroing_and_base_build; eauto; lia.
...
Lemma proof_of_max_edit_string_matches_entail_wit_4_split_goal_8 : max_edit_string_matches_entail_wit_4_split_goal_8.
Proof.
  pre_process.
  dump_pre_spatial.
  eapply EditZeroPrefix_to_full_at_bound__zeroing_and_base_build; eauto; lia.
...
Lemma proof_of_max_edit_string_matches_entail_wit_4_split_goal_9 : max_edit_string_matches_entail_wit_4_split_goal_9.
Proof.
  pre_process.
  dump_pre_spatial.
  eapply EditZeroPrefix_to_full_at_bound__zeroing_and_base_build; eauto; lia.
...
Proof.
  aggressive_pre_process.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_4_split_goal_1.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_4_split_goal_2.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_4_split_goal_3.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_4_split_goal_4.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_4_split_goal_5.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_4_split_goal_6.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_4_split_goal_7.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_4_split_goal_8.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_4_split_goal_9.
Qed.
...
Ltac solve_max_edit_string_matches_entail_wit_8_1_bound :=
  pre_process; dump_pre_spatial;
  let Hseg := fresh "Hseg" in
  let Hlast := fresh "Hlast" in
  match goal with
  | Hbuild : EditBuildState _ _ ?n ?i0 ?sg ?c10 ?c11,
    Hscratch : EditScratchCountsBound _ _ _ _ _ |- _ =>
      destruct Hbuild as [_ [_ [_ [Hseg _]]]];
      match goal with
      | |- context [Znth ?pos (?sg0 ++ ?x :: nil) 0] =>
          assert (Hlast : Znth pos (sg0 ++ x :: nil) 0 = x)
            by (rewrite app_Znth2 by (rewrite (proj1 Hseg); lia); rewrite (proj1 Hseg);
                replace (pos - i0) with 0 by lia; reflexivity)
      end;
      rewrite Hlast in *;
      ...
      destruct Hscratch as [[_ Hcnt10] [[_ Hcnt11] _]];
      first
        [ match type of Hcnt10 with
          | forall idx, _ -> 0 <= Znth idx ?cnt10 0 <= _ =>
              match goal with
              | |- context [Znth ?idx cnt10 0] =>
                  pose proof (Hcnt10 idx ltac:(lia)); lia
              end
          end
        | match type of Hcnt11 with
          | forall idx, _ -> 0 <= Znth idx ?cnt11 0 <= _ =>
              match goal with
              | |- context [Znth ?idx cnt11 0] =>
                  pose proof (Hcnt11 idx ltac:(lia)); lia
              end
          end
        | lia ]
  | Hseg : EditSegmentPrefix _ ?i0 ?sg,
    Hcnt10 : forall idx, 0 <= idx < ?n -> 0 <= Znth idx ?c10 0 <= ?n,
    Hcnt11 : forall idx, 0 <= idx < ?n -> 0 <= Znth idx ?c11 0 <= ?n |- _ =>
      match goal with
      | |- context [Znth ?pos (?sg0 ++ ?x :: nil) 0] =>
          assert (Hlast : Znth pos (sg0 ++ x :: nil) 0 = x)
            by (rewrite app_Znth2 by (rewrite (proj1 Hseg); lia); rewrite (proj1 Hseg);
      ...
      pose proof
        (EditSegmentPrefix_last_block_bounds__build_s1_segments_counts
           _ _ _ Hseg ltac:(lia)) as Hblock_bounds;
      try (apply Hcnt10; lia);
      try (apply Hcnt11; lia);
      try lia
  end.

Ltac solve_max_edit_string_matches_bound_from Hbuild Hscratch :=
  pre_process; dump_pre_spatial;
  let Hseg := fresh "Hseg" in
  let Hlast := fresh "Hlast" in
  let Hblock_bounds := fresh "Hblock_bounds" in
  let Hcnt10 := fresh "Hcnt10" in
  let Hcnt11 := fresh "Hcnt11" in
  destruct Hbuild as [_ [_ [_ [Hseg _]]]];
  destruct Hscratch as [[_ Hcnt10] [[_ Hcnt11] _]];
  match goal with
  | |- context [Znth ?pos (?sg0 ++ ?x :: nil) 0] =>
      assert (Hlast : Znth pos (sg0 ++ x :: nil) 0 = x)
        by (rewrite app_Znth2 by (rewrite (proj1 Hseg); lia); rewrite (proj1 Hseg);
            replace (pos - Zlength sg0) with 0 by lia; reflexivity)
  end;
  rewrite Hlast in *;
  repeat match goal with
  | |- context [?x - 0] => replace (x - 0) with x by lia
...
Ltac solve_max_edit_string_matches_prev_bound Hbuild Hscratch i sg :=
  pre_process; dump_pre_spatial;
  let Hseg := fresh "Hseg" in
  let Hlast := fresh "Hlast" in
  let Hblock_bounds := fresh "Hblock_bounds" in
  ...
  destruct Hscratch as [[_ Hcnt10] [[_ Hcnt11] _]];
  assert (Hlast :
    Znth i (sg ++ Znth ((i - 1) - 0) sg 0 :: nil) 0 =
    Znth ((i - 1) - 0) sg 0);
  ...
  | rewrite Hlast;
    ...
    pose proof
      (EditSegmentPrefix_last_block_bounds__build_s1_segments_counts
         _ _ _ Hseg ltac:(lia)) as Hblock_bounds;
    pose proof (Hcnt10 (Znth (i - 1) sg 0) ltac:(lia));
    pose proof (Hcnt11 (Znth (i - 1) sg 0) ltac:(lia));
    lia ].

Ltac solve_max_edit_string_matches_new_bound Hbuild Hscratch i sg :=
  pre_process; dump_pre_spatial;
  let Hseg := fresh "Hseg" in
  let Hlast := fresh "Hlast" in
  let Hcnt10 := fresh "Hcnt10" in
  ...
  destruct Hbuild as [_ [_ [_ [Hseg _]]]];
  destruct Hscratch as [[_ Hcnt10] [[_ Hcnt11] _]];
  assert (Hlast : Znth i (sg ++ i :: nil) 0 = i);
  [ rewrite app_Znth2 by (rewrite (proj1 Hseg); lia);
    rewrite (proj1 Hseg);
    replace (i - i) with 0 by lia;
    reflexivity
...
Proof. solve_max_edit_string_matches_prev_bound PreH13 PreH16 i sg1_2. Qed.
...
Ltac solve_max_edit_string_matches_entail_wit_8_2_bound :=
  pre_process; dump_pre_spatial;
  let Hseg := fresh "Hseg" in
  let Hlast := fresh "Hlast" in
  match goal with
  | Hbuild : EditBuildState _ _ ?n ?i0 ?sg ?c10 ?c11,
    Hscratch : EditScratchCountsBound ?n ?c10 ?c11 _ _ |- _ =>
      destruct Hbuild as [_ [_ [_ [Hseg _]]]];
      match goal with
      | |- context [Znth ?pos (?sg0 ++ ?x :: nil) 0] =>
          assert (Hlast : Znth pos (sg0 ++ x :: nil) 0 = x)
...
Proof. solve_max_edit_string_matches_new_bound PreH12 PreH15 i sg1_2. Qed.
...
Ltac solve_max_edit_string_matches_entail_wit_8_3_bound :=
  pre_process; dump_pre_spatial;
  let Hseg := fresh "Hseg" in
  let Hlast := fresh "Hlast" in
  let Hblock := fresh "Hblock" in
  match goal with
  | Hbuild : EditBuildState _ ?t1 ?n ?i0 ?sg ?c10 ?c11,
    Hscratch : EditScratchCountsBound ?n ?c10 ?c11 _ _ |- _ =>
      destruct Hbuild as [_ [_ [_ [Hseg _]]]];
      replace (i0 - 1 - 0) with (i0 - 1) by lia;
      assert (Hlast :
        Znth i0 (sg ++ Znth (i0 - 1) sg 0 :: nil) 0 =
        Znth (i0 - 1) sg 0)
        by (eapply edit_snoc_Znth_last__build_s1_segments_counts;
            [exact (proj1 Hseg)|lia]);
      assert (Hblock : 0 <= Znth (i0 - 1) sg 0 < n)
        by (pose proof (EditSegmentPrefix_last_block_bounds__build_s1_segments_counts
                          t1 i0 sg Hseg ltac:(lia)); lia);
      rewrite Hlast;
      unfold EditScratchCountsBound, EditCountBounds in Hscratch;
      destruct Hscratch as [[_ Hcnt10] [[_ Hcnt11] _]];
      try (apply Hcnt10; lia);
...
Proof. solve_max_edit_string_matches_new_bound PreH13 PreH16 i sg1_2. Qed.
...
Ltac solve_max_edit_string_matches_entail_wit_9_split_goal :=
  pre_process; dump_pre_spatial;
  repeat match goal with
  | |- context[?x - 0] => replace (x - 0) with x by lia
  end;
...
Proof. solve_max_edit_string_matches_entail_wit_9_split_goal. Qed.
...
Lemma proof_of_max_edit_string_matches_entail_wit_12_split_goal_1 : max_edit_string_matches_entail_wit_12_split_goal_1.
Proof.
  pre_process.
Qed. 

Lemma proof_of_max_edit_string_matches_entail_wit_12_split_goal_2 : max_edit_string_matches_entail_wit_12_split_goal_2.
Proof.
  pre_process.
Qed.

Lemma proof_of_max_edit_string_matches_entail_wit_12_split_goal_3 : max_edit_string_matches_entail_wit_12_split_goal_3.
Proof.
  pre_process.
Qed.

Lemma proof_of_max_edit_string_matches_entail_wit_12_split_goal_4 : max_edit_string_matches_entail_wit_12_split_goal_4.
Proof.
  pre_process.
Qed.

Lemma proof_of_max_edit_string_matches_entail_wit_12_split_goal_5 : max_edit_string_matches_entail_wit_12_split_goal_5.
Proof.
  pre_process.
...
Proof.
  aggressive_pre_process.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_12_split_goal_1.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_12_split_goal_2.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_12_split_goal_3.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_12_split_goal_4.
  - Goal_apply proof_of_max_edit_string_matches_entail_wit_12_split_goal_5.
Qed. 
...
Proof.
  ...
  - sep_apply (IntArray.seg_single seg2_pre 0 0).
    entailer!.
  - unfold EditZeroFull in PreH10.
    destruct PreH10 as [_ Hzero].
  ...
  - unfold EditZeroFull in PreH10.
    ...
    rewrite Hzero by lia.
    lia.
  - unfold EditZeroFull in PreH9.
    destruct PreH9 as [_ Hzero].
    rewrite Hzero by lia.
    lia.
  - unfold EditZeroFull in PreH9.
    destruct PreH9 as [_ Hzero].
...
Ltac solve_max_edit_string_matches_s2_seg_index_bound :=
  match goal with
...
Ltac solve_max_edit_string_matches_s2_app_bound :=
  match goal with
...
Ltac solve_max_edit_string_matches_s2_counts :=
  match goal with
  | Hbuild : EditBuildState _ _ _ _ _ _ _ |- EditCountsForPrefix _ _ _ _ _ _ =>
      destruct Hbuild as [_ [_ [_ [_ Hcounts]]]];
      exact Hcounts
  end.

Ltac solve_max_edit_string_matches_s2_segment :=
  match goal with
  | Hbuild : EditBuildState _ ?t _ ?i ?sg _ _
      |- EditSegmentPrefix ?t (?i + 1) (?sg ++ Znth (?i - 1 - 0) ?sg 0 :: nil) =>
      replace (i - 1 - 0) with (i - 1) by lia;
      destruct Hbuild as [_ [_ [_ [Hseg _]]]];
      eapply EditSegmentPrefix_extend_open__build_s1_segments_counts; eauto; try lia;
      unfold edit_edge_open; split; assumption
...
Proof.
  right. intros. pre_process. entailer!;
    try solve [solve_max_edit_string_matches_s2_seg_index_bound];
    try solve [solve_max_edit_string_matches_s2_app_bound];
    try solve [solve_max_edit_string_matches_s2_counts];
    try solve [solve_max_edit_string_matches_s2_segment].
Qed.
...
Proof.
  ...
  entailer!.
  all: replace (i - 0) with i by lia; auto.
Qed. 
...
Proof.
  ...
  Exists sg1_2 sg2_2 c10_2 c11_2 c20_2 c21_2.
  entailer!.
  - intros idx Hidx. specialize (HBt2 idx Hidx). destruct HBt2; subst; lia.
  - intros idx Hidx. specialize (HBt1 idx Hidx). destruct HBt1; subst; lia.
  - intros idx Hidx. specialize (HBs2 idx Hidx). destruct HBs2; subst; lia.
  - intros idx Hidx. specialize (HBs1 idx Hidx). destruct HBs1; subst; lia.
Qed.

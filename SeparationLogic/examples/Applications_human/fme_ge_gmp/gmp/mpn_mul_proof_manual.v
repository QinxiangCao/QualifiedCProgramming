...
Proof.
  unfold mpn_mul_entail_wit_1.
  left.
  intros vn_pre vp_pre un_pre up_pre rp_pre l_vp l_src
    Hun_pos Hvn_pos Hvn_le Hlen_src Hlen_vp Hbound_src Hbound_vp.
  pose proof Hlen_vp as H4.
  pose proof Hbound_vp as H6.
  Intros.
  prop_apply
  ...
  - Intros.
    entailer!.
    unfold UINT_MOD in Hvp0_bound.
    lia.
Qed.

Lemma proof_of_mpn_mul_entail_wit_2 : mpn_mul_entail_wit_2.
Proof.
  unfold mpn_mul_entail_wit_2.
  left.
  intros vn_pre vp_pre un_pre up_pre rp_pre l_vp l_src val_out l_out retval
    Hlen_out Hbound_out Hval_out Hmul_eq Hun_pos Hvn_pos Hvn_le Hsum_max
    Hlen_src Hlen_vp Hbound_src Hbound_vp Hvp0_nonneg Hvp0_max.
  pose proof Hlen_out as H.
  pose proof Hbound_out as H0.
  pose proof Hval_out as H1.
  pose proof Hmul_eq as H2.
  pose proof Hlen_src as H7.
  pose proof Hlen_vp as H8.
  pose proof Hbound_src as H9.
  Exists (l_out ++ retval :: nil).
  Intros.
  pose proof (list_to_Z_bound UINT_MOD UINT_MOD_pos l_out H0) as Hout_bound.
  rewrite H in Hout_bound.
  pose proof (list_to_Z_bound UINT_MOD UINT_MOD_pos l_src H9) as Hsrc_bound.
  rewrite H7 in Hsrc_bound.
  ...
  { rewrite <- H1 in H2.
    ...
    unfold UINT_MOD in *.
    nia. }
  assert (Hdone_len: Zlength (l_out ++ retval :: nil) = un_pre + 1).
  { rewrite Zlength_app. rewrite Zlength_cons. rewrite Zlength_nil. lia. }
  assert (Hdone_bound: list_within_bound UINT_MOD (l_out ++ retval :: nil)).
  ...
  { replace 1 with (0 + 1) by lia.
    ...
    rewrite H8. lia. }
  assert (Heq_done:
    list_to_Z UINT_MOD (l_out ++ retval :: nil) =
    list_to_Z UINT_MOD l_src * list_to_Z UINT_MOD (sublist 0 1 l_vp)).
  { rewrite (list_to_Z_concat_r UINT_MOD UINT_MOD_pos l_out retval Hretval_bound).
    rewrite Hsub01.
...
Proof.
  unfold mpn_mul_entail_wit_3.
  left.
  intros vn_pre vp_pre un_pre up_pre rp_pre l_vp l_src l_done_2 cy i
    Hi_lt Hun_pos Hvn_pos Hvn_le Hun_max Hsum_max Hi_ge Hi_le Hcy_nonneg Hcy_max
    Hlen_done Hlen_src Hlen_vp Hbound_done Hbound_src Hbound_vp Hdone_val.
  pose proof Hlen_done as H9.
  pose proof Hbound_done as H12.
  Exists l_done_2.
  ...
  { apply list_within_bound_sublist; try lia; exact H12. }
...
Proof.
  unfold mpn_mul_entail_wit_4.
  left.
  intros vn_pre vp_pre un_pre up_pre rp_pre l_vp l_src l_done_2 l_low_2 l_mid_2 i cy
    Hun_pos Hvn_pos Hvn_le Hun_max Hsum_max Hi_ge Hi_lt Hcy_nonneg Hcy_max
    Hlen_low Hlen_mid Hdone_app Hlen_done Hlen_src Hlen_vp Hbound_done
    Hbound_low Hbound_mid Hbound_src Hbound_vp Hdone_val.
  pose proof Hlen_vp as H13.
  pose proof Hbound_vp as H18.
  Exists l_done_2.
  Exists l_mid_2.
  Exists l_low_2.
  Intros.
  ...
  - Intros.
    entailer!.
    unfold UINT_MOD in Hvl_bound.
    lia.
Qed.

Lemma proof_of_mpn_mul_entail_wit_5 : mpn_mul_entail_wit_5.
Proof.
  unfold mpn_mul_entail_wit_5.
  left.
  intros vn_pre vp_pre un_pre up_pre rp_pre l_vp l_src i cy vl rp_i l_done_2 l_low l_mid val_out l_out retval
    Hlen_out Hbound_out Hval_out Hmul_eq Hun_pos Hvn_pos Hvn_le Hun_max Hsum_max
    Hi_ge Hi_lt Hcy_nonneg Hcy_max Hvl_nonneg Hvl_max Hlen_low Hlen_mid Hdone_app
    Hlen_done Hlen_src Hlen_vp Hbound_done Hbound_low Hbound_mid Hbound_src Hbound_vp
    Hdone_val Hrp_i_eq Hvl_eq.
  pose proof Hlen_out as H.
  pose proof Hbound_out as H0.
  pose proof Hval_out as H1.
  pose proof Hmul_eq as H2.
  pose proof Hlen_low as H14.
  pose proof Hlen_mid as H15.
  pose proof Hdone_app as H16.
  pose proof Hlen_src as H18.
  pose proof Hlen_vp as H19.
  pose proof Hbound_low as H21.
  pose proof Hbound_mid as H22.
  pose proof Hbound_src as H23.
  pose proof Hbound_vp as H24.
  pose proof Hdone_val as H25.
  pose proof Hrp_i_eq as H26.
  pose proof Hvl_eq as H27.
  Exists ((l_low ++ l_out) ++ retval :: nil).
  Intros.
  pose proof (list_to_Z_bound UINT_MOD UINT_MOD_pos l_out H0) as Hout_bound.
  rewrite H in Hout_bound.
  pose proof (list_to_Z_bound UINT_MOD UINT_MOD_pos l_mid H22) as Hmid_bound.
  rewrite H15 in Hmid_bound.
  pose proof (list_to_Z_bound UINT_MOD UINT_MOD_pos l_src H23) as Hsrc_bound.
  rewrite H18 in Hsrc_bound.
  ...
  { apply list_within_bound_concat.
    - apply list_within_bound_concat; [exact H21 | exact H0].
    - simpl. split; [exact Hretval_bound | tauto]. }
  assert (Hdone2_to_Z:
  ...
  { rewrite (list_to_Z_concat_r UINT_MOD UINT_MOD_pos (l_low ++ l_out) retval Hretval_bound).
    ...
    rewrite H1.
    rewrite Zlength_app. rewrite H14. rewrite H.
...
Proof.
  unfold mpn_mul_return_wit_1.
  left.
  intros vn_pre vp_pre un_pre up_pre rp_pre l_vp l_src l_done cy i
    Hi_ge Hun_pos Hvn_pos Hvn_le Hun_max Hsum_max Hi_lower Hi_le Hcy_nonneg Hcy_max
    Hlen_done Hlen_src Hlen_vp Hbound_done Hbound_src Hbound_vp Hdone_val.
  pose proof Hdone_val as H15.
  ...
  - rewrite (UIntArray.undef_seg_empty rp_pre (un_pre + vn_pre)).
    ...
    entailer!.
  - Intros.
    rewrite (sublist_self l_vp vn_pre) in H15 by lia.
    entailer!.
Qed.

...
Proof.
  unfold mpz_div_qr_safety_wit_35_nonalias.
  left.
  pre_process.
  prop_apply (store_int_range (&("nn")) retval).
  Intros.
  prop_apply (store_int_range (&("dn")) retval_2).
  Intros.
...
Proof.
  unfold mpz_div_qr_entail_wit_4_r_eq_n.
  left.
  intros; Intros.
  prop_apply
...
Proof.
  pre_process; Exists 0 zn_r_eq_n.
  ...
  { eapply (mpz_div_qr_math_small_remainder
    ...
    - assumption.
    - exact Habs_lt. }
  sep_apply (store_Z_from_fields d0_r_eq_n ptr size_2 cap_2 zd_r_eq_n);
    [ | entailer! | entailer!].
  sep_apply (store_Z_zero_from_fields q0_r_eq_n ptr_2 size_3 cap_3 old_q_r_eq_n);
    [ | entailer! | entailer!].
  sep_apply (store_Z_to_optional_store_Z_nonzero q0_r_eq_n 0); [ | entailer!].
  entailer!.
  unfold mpz_div_qr_ret_ok; lia.
Qed.
...
Proof.
  ...
  sep_apply (store_Z_from_fields d0_r_eq_n ptr size_2 cap_2 zd_r_eq_n);
    [ | entailer! | entailer!].
  sep_apply (optional_store_Z_null_change q0_r_eq_n old_q_r_eq_n 0); [ | entailer!].
  entailer!.
  unfold mpz_div_qr_ret_ok; lia.
Qed.
...
Proof.
  ...
  subst r0_r_eq_n.
  subst size.
  prop_apply_p (mpd_store_Z_compact_zero UINT_MOD ptr (Zabs zn_r_eq_n)).
  Intros.
  assert (Hzn0 : zn_r_eq_n = 0) by lia.
  subst zn_r_eq_n.
  change (Zabs 0) with 0.
  sep_apply (store_Z_from_zero_fields n0_r_eq_n ptr cap); [ | entailer!].
  sep_apply (store_Z_from_fields d0_r_eq_n ptr_2 size_2 cap_2 zd_r_eq_n);
    [ | entailer! | entailer!].
  sep_apply (store_Z_zero_from_fields q0_r_eq_n ptr_3 size_3 cap_3 old_q_r_eq_n);
    [ | entailer! | entailer!].
  sep_apply (store_Z_to_optional_store_Z_nonzero q0_r_eq_n 0); [ | entailer!].
  entailer!.
  - unfold mpz_div_qr_ret_ok; lia.
...
Proof.
  unfold mpz_div_qr_partial_solve_wit_12_r_eq_n_pure.
  right; intros; split_pures.
  - dump_pre_spatial.
    destruct (Z_lt_ge_dec size 0).
    + rewrite Z.abs_neq in PreH17 by lia; lia.
...
Proof.
  unfold mpz_div_qr_partial_solve_wit_14_r_eq_n_pure.
  right; intros; split_pures.
  - dump_pre_spatial.
    pose proof (is_compact_Z_positive_size (Zabs zd_r_eq_n) (Zabs size)
      PreH16 ltac:(pose proof (Z.abs_nonneg zd_r_eq_n); lia)).
    destruct (Z_lt_ge_dec size 0).
    + rewrite Z.abs_neq in PreH18 by lia; lia.
    + lia.
Qed.

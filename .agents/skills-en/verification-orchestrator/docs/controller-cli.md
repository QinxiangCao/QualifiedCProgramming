...
| `--freeze-spec` | no, repeatable or comma-separated | Freeze the specification of these C functions; omit to let the annotation agent author specifications freely |
...
**`--freeze-spec`:** Treats the specification as a fixed input for this run rather than something to redesign. For each named function, its `With` / `Require` / `Ensure` blocks -- including named specs and any `<= other_spec` refinement clause -- must survive the annotation phase token-identical. Comments, whitespace, and line endings are formatting, not modification.

Using the flag at all also freezes every existing `Extern Coq` entry, `Import Coq` module, and case-lib top-level declaration: a frozen specification's meaning depends on those definitions, so without them the text could stay fixed while the semantics drifted. **Additions are unrestricted** -- new `Extern Coq` entries, new imports, new case-lib definitions and lemmas, and specifications for functions not listed. `Inv Assert` and `Assert` remain fully editable.

Init extracts the baseline into `spec_freeze` in `controller_state.json`. `annotation-check-round` re-extracts and compares entry by entry; any changed or removed entry fails the attempt and returns to the annotation agent as `annotation-main-check-spec-freeze` with the offending entry named. Omitting the flag performs no comparison and preserves the previous behavior exactly.
...
**Operation and result:** Validates owner, seals delivery, and directly performs annotation/vc-checking phase validation or group validation. Results may include `ready-for-main-check`, `returned`, `report-repair-required`, `invalid-report`, group-validation output, or an idempotent result. Finalize seals every file the owner actually delivered; a valid group blocker with `failure_class: annotation-gap` additionally requires a delivered `group_worker_output.md` that is a nonempty UTF-8 regular file at its fixed path. The controller checks structure/ownership/route/helper/import/safety with `require_complete=False`, does not run exact/full group Rocq, and makes it a blocked terminal state and reuse source only when structurally valid. It neither asks the owner to modify annotations outside its boundary nor stops sibling scheduling.
...
When the run was started with `--freeze-spec`, this command also re-extracts the specification surface after the `formal_case_lib` contract check and before acceptance, comparing it entry by entry against `spec_freeze.baseline`. A changed or removed entry ends the attempt as `main-check-failed`, records `mismatch_count` and `first_mismatch` (with `section`, `entry`, and `kind`) under `main_check.spec_freeze`, and returns to the same annotation agent as `annotation-main-check-spec-freeze`. Runs without the flag skip this comparison.

...
Proof.
  unfold build_aug_basis_gmp_entail_wit_3_1.
  intros.
  ...
  - split_pures; try exact Hpart_next; try lia;
      apply dump_spatial_left; try assumption; try lia.
Qed.

Lemma proof_of_build_aug_basis_gmp_entail_wit_3_2 : build_aug_basis_gmp_entail_wit_3_2.
Proof.
  unfold build_aug_basis_gmp_entail_wit_3_2.
  intros.
  set (l_next := replace_Znth idx 1 laug_cur_2).
  assert (Hpart_next :
    partial_rep_matrix l_next n_pre cols (idx + 1)
...
Proof.
  unfold build_aug_basis_gmp_entail_wit_3_3.
  intros.
...
Proof.
  unfold build_aug_basis_gmp_entail_wit_3_4.
  intros.
  Left. Right.
  set (l_next := replace_Znth idx 0 laug_cur_2).
  assert (Hpart_next :
...
Proof.
  unfold build_aug_basis_gmp_entail_wit_3_5.
  intros.
...
Proof.
  unfold build_aug_basis_gmp_entail_wit_4_1.
  left.
  intros.
...
Proof.
  unfold build_aug_basis_gmp_entail_wit_4_2.
  left.
  intros.
  assert (Hj_cols : j = cols) by lia.
  assert (Hidx_next : idx = (i + 1) * cols).
...
Proof.
  unfold build_aug_basis_gmp_entail_wit_4_3.
  left.
  intros.
  assert (Hj_cols : j = cols) by lia.
  assert (Hidx_next : idx = (i + 1) * cols).
...
Proof.
  unfold build_aug_basis_gmp_entail_wit_4_4.
  left.
  intros.
  assert (Hj_cols : j = cols) by lia.
  assert (Hidx_next : idx = (i + 1) * cols).
...
Proof.
  unfold build_aug_basis_gmp_return_wit_1.
  left.
  intros.

...
Lemma proof_of_mpn_copyi_entail_wit_1_off_zero : mpn_copyi_entail_wit_1_off_zero.
Proof.
  pre_process.
  subst off_shifted.
...
Lemma proof_of_mpn_copyi_entail_wit_2_off_pos : mpn_copyi_entail_wit_2_off_pos.
Proof.
  pre_process.
  sep_apply UIntArray.full_to_seg.
...
Proof.
  unfold mpn_copyi_entail_wit_3_off_zero.
  right; intros.
...
Lemma proof_of_mpn_copyi_entail_wit_6_off_pos : mpn_copyi_entail_wit_6_off_pos.
Proof.
  aggressive_pre_process.
  subst s_pre.
...
Lemma proof_of_mpn_copyi_return_wit_1_off_zero : mpn_copyi_return_wit_1_off_zero.
Proof.
  aggressive_pre_process.
  ...
  entailer!.
  all: assert (i = n_pre) by lia.
  all: subst i; subst off_shifted.
  all: replace (0 + n_pre) with n_pre by lia.
  all: rewrite (Zsublist_nil l_shifted n_pre n_pre) by lia.
...
Proof.
  ...
  entailer!.
  all: rewrite Zlength_sublist by lia; reflexivity.
Qed.
...
Proof.
  unfold mpz_div_q_2exp_entail_wit_3_4.
  right.
  intros.
  Exists (Z.max qn cap) qp.
...
Lemma proof_of_mpz_div_q_2exp_partial_solve_wit_5_pure : mpz_div_q_2exp_partial_solve_wit_5_pure.
Proof.
  pre_process.
  assert (Hlimb_nonneg : 0 <= bit_index_pre ÷ 32).
  { apply Z.quot_pos; lia. }
...
Lemma proof_of_mpz_div_q_2exp_partial_solve_wit_7_pure : mpz_div_q_2exp_partial_solve_wit_7_pure.
Proof.
  pre_process.
  assert (Hlimb_nonneg : 0 <= bit_index_pre ÷ 32).
  { apply Z.quot_pos; lia. }

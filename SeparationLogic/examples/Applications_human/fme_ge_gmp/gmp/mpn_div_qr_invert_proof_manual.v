...
Proof.
  unfold mpn_div_qr_invert_entail_wit_1.
  left.
  intros.
...
Lemma proof_of_mpn_div_qr_invert_entail_wit_3 : mpn_div_qr_invert_entail_wit_3.
Proof.
  pre_process.
...
Lemma proof_of_mpn_div_qr_invert_entail_wit_4 : mpn_div_qr_invert_entail_wit_4.
Proof.
  unfold mpn_div_qr_invert_entail_wit_4.
  right.
  intros.
  unfold div_inverse_slot.
  entailer!.
...
Lemma proof_of_mpn_div_qr_invert_entail_wit_5 : mpn_div_qr_invert_entail_wit_5.
Proof.
  pre_process.
...
Lemma proof_of_mpn_div_qr_invert_entail_wit_6_1 : mpn_div_qr_invert_entail_wit_6_1.
Proof.
  pre_process.
  ...
  pose proof (mpn_lshift_high2_quot_uint
    l_dp dn_pre retval PreH14 Hdn_gt2 PreH17 ltac:(lia) Htop_no_over)
    as Hhigh2.
  assert (Hd1range:
    0 <=
      Z.lor
    ...
    <= 4294967295).
  {
      ...
      0 <= Znth (dn_pre - 2) l_dp 0 <= 4294967295).
    {
      pose proof (list_within_bound_Znth_bound
        UINT_MOD l_dp (dn_pre - 2) ltac:(rewrite PreH14; lia) PreH17).
    ...
    <= 4294967295).
  {
    ...
    {
      pose proof (list_within_bound_Znth_bound
        UINT_MOD l_dp (dn_pre - 3) ltac:(rewrite PreH14; lia) PreH17).
      unfold UINT_MOD in *; lia.
    }
    pose proof (mpn_lshift_join_limb_bound
      (unsigned_last_nbits
      ...
      retval_2).
  {
    ...
    - pose proof (list_to_Z_bound UINT_MOD UINT_MOD_pos l_dp PreH17) as Hbound.
      rewrite PreH14, PreH15 in Hbound.
        ...
        d / UINT_MOD ^ (dn_pre - 1) = Znth (dn_pre - 1) l_dp 0).
      {
        pose proof (list_to_Z_high_limb_quot_uint_for_div
          l_dp dn_pre PreH14 ltac:(lia) PreH17) as Hq.
        rewrite PreH15 in Hq.
        exact Hq.
      }
  ...
  destruct (Z.eq_dec dn_pre 1); [lia|].
  sep_apply (store_uint_undef_store_uint &("d0")
    (Z.lor
      (unsigned_last_nbits
        (Z.shiftl (Znth (dn_pre - 2) l_dp 0) retval) 32)
      (Z.shiftr (Znth (dn_pre - 3) l_dp 0) (32 - retval)))).
  sep_apply (store_uint_undef_store_uint &("d1")
    (Z.lor
      (unsigned_last_nbits
        (Z.shiftl (Znth (dn_pre - 1) l_dp 0) retval) 32)
      (Z.shiftr (Znth (dn_pre - 2) l_dp 0) (32 - retval)))).
  sep_apply (store_uint_undef_store_uint &("shift") retval).
  entailer!.
Qed.

Lemma proof_of_mpn_div_qr_invert_entail_wit_6_2 : mpn_div_qr_invert_entail_wit_6_2.
Proof.
  pre_process.
  assert (Hdn_gt2: dn_pre > 2) by lia.
  ...
  pose proof (list_to_Z_high2_quot_uint_for_div
    l_dp dn_pre PreH14 ltac:(lia) PreH17) as Hhigh2.
  assert (Hd1range: 0 <= Znth (dn_pre - 1) l_dp 0 <= 4294967295).
  {
    pose proof (list_within_bound_Znth_bound
      UINT_MOD l_dp (dn_pre - 1) ltac:(rewrite PreH14; lia) PreH17).
    unfold UINT_MOD in *; lia.
  }
      ...
      retval_2).
  {
    ...
    - change (UINT_MOD / 2) with (2 ^ 31).
      ...
      replace (Znth (dn_pre - 1) l_dp 0 * 1)
        with (Znth (dn_pre - 1) l_dp 0) in PreH8 by ring.
      exact PreH8.
    - unfold div_inverse_den.
      destruct (Z.eq_dec dn_pre 1); [lia|].
      exact PreH3.
  ...
  sep_apply (store_uint_undef_store_uint &("d1")
    (Znth (dn_pre - 1) l_dp 0)).
  sep_apply (store_uint_undef_store_uint &("shift") 0).
  entailer!.
Qed.
